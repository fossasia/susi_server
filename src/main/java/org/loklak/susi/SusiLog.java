...
public class SusiLog {
    ...
    static {
        Collections.addAll(failset, failterms);
        Collections.addAll(dnruset, donotremoveunanswered);
    }
    ...
    public ArrayList<SusiInteraction> getInteractions(String client) {
        UserRecord ur = this.logs.computeIfAbsent(client, k -> new UserRecord(client));
    ...
    private static ArrayList<SusiInteraction> readLog(final File logdump, int count) throws IOException {
    ...
    public JsonTray getRuleset(String client, String name) throws IOException {
        Map<String, JsonTray> rulesets = this.rulesets.computeIfAbsent(client, k -> new HashMap<>());
