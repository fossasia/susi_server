...
public class SusiPhrase {
    ...
    public SusiPhrase(JSONObject json) throws PatternSyntaxException {
        ...
        if (json.has("type")) try {
            ...
            t = expression.contains(".*") ? Type.regex : expression.indexOf('*') >= 0 ? Type.pattern : Type.minor;
        }
        
        expression = normalizeExpression(expression);
        if ((t == Type.minor || t == Type.prior) && expression.contains(".*")) t = Type.regex;
