...
public class SusiAction {
    ...
    public enum RenderType {answer, table, piechart, rss, self, websearch, anchor, map}
    public enum SelectionType {random, roundrobin}
    ...
    private static DialogType getDialogType(Collection<String> phrases) {
        DialogType type;
    ...
    private static DialogType getDialogType(String phrase) {
        if (phrase.endsWith("?")) {
            return phrase.contains(". ") ? DialogType.reply : DialogType.question;
        }
        return DialogType.answer;
    }
