...
public abstract class AbstractAPIHandler extends HttpServlet implements APIHandler {
    ...
    protected static final Long defaultCookieTime = (long) (60 * 60 * 24 * 7);
    private static final Long defaultAnonymousTime = (long) (60 * 60 * 24);
    ...
    protected abstract JSONObject serviceImpl(Query call, HttpServletResponse response, Authorization rights, final JSONObjectWithDefault permissions) throws APIException;
    ...
    private void process(HttpServletRequest request, HttpServletResponse response, Query query) throws ServletException, IOException {
        ...
        Accounting accounting_temporary = DAO.accounting_temporary.computeIfAbsent(identity.toString(), k -> new Accounting());
    ...
    private static ClientIdentity getIdentity(HttpServletRequest request, HttpServletResponse response, Query query) {
    	...
    	if(getLoginCookie(request) != null){ // check if login cookie is set
			...
			ClientCredential credential = new ClientCredential(ClientCredential.Type.cookie, loginCookie != null ? loginCookie.getValue() : null);
     ...
     * @return String hash
     */
    protected static String getHash(String input, String salt){
		try {
