...
public class JsonRepository {
    ...
    private final static byte[] OPERATION_KEY = "$P".getBytes();
    private final static byte[] MOD_DATE_KEY  = "$D".getBytes();
    private final static byte[] REFERRER_KEY  = "$U".getBytes();
    ...
    private final File dump_dir;
    private final File dump_dir_own;
    private final File dump_dir_import;
    private final File dump_dir_imported;
    private final File dump_dir_buffer;
    private final String dump_file_prefix;
    private final JsonRandomAccessFile json_log;
    ...
    public JsonFactory write(JSONObject json, char opkey) throws IOException {
        String line = json.toString(); // new ObjectMapper().writer().writeValueAsString(map);
        JsonFactory jf;
        String sb = "{" + '\"' + UTF8.String(OPERATION_KEY) + '\"' + ':' + '\"' + opkey + '\"' + ',' +
                line.substring(1);
        byte[] b = UTF8.getBytes(sb);
        long seekpos = this.json_log.appendLine(b);
        jf = this.json_log.getJsonFactory(seekpos, b.length);
        return jf;
    }
    ...
    private static SortedSet<File> getDumps(final File path, final String prefix, final String suffix, int count) {
        ...
        TreeSet<File> dumps = new TreeSet<>(); // sort the names with a tree set
        for (String s: list) {
