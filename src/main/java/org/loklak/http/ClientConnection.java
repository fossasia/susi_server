...
public class ClientConnection {
     ...
     * @throws IOException
     */
    private ClientConnection(String urlstring, boolean useAuthentication) throws IOException {
     ...
     * @throws IOException
     */
    private ClientConnection(String urlstring, Map<String, byte[]> map, boolean useAuthentication) throws IOException {
    	this.httpClient = HttpClients.custom()
			.useSystemProperties()
			.setConnectionManager(getConnctionManager(useAuthentication))
			.setDefaultRequestConfig(defaultRequestConfig)
        ...
        if (httpEntity != null) {
            if (this.httpResponse.getStatusLine().getStatusCode() == 200) {
                ...
                for (Header header: httpResponse.getAllHeaders()) {
                    List<String> vals = this.header.computeIfAbsent(header.getName(), k -> new ArrayList<>());
     ...
     */
    private static String getRedirect(String urlstring, boolean useAuthentication) throws IOException {
    ...
    private static void download(String source_url, File target_file, boolean useAuthentication) {
        try {
            ClientConnection connection = new ClientConnection(source_url, useAuthentication);
            try {
                int count;
                byte[] buffer = new byte[2048];
                try (OutputStream os = new BufferedOutputStream(new FileOutputStream(target_file))) {
    ...
    private static byte[] download(String source_url, boolean useAuthentication) throws IOException {
        try {
