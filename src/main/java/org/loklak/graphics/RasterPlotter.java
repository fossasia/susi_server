...
public class RasterPlotter {
    ...
    private static final double TL = Math.sqrt(3) / 2;

    // colors regarding RGB Color Model
    private static final long RED    = 0xFF0000;
    private static final long GREEN  = 0x00FF00;
    private static final long BLUE   = 0x0000FF;
    private static final long GREY   = 0x888888;
    ...
    private static boolean darkColor(final long c) {
    ...
    private int[] getPixel(final int x, final int y, int[] c) {
    ...
    private void setPixel(final int x, final int y, int[] c) {
        if (this.frame == null) {
    ...
    void line(final int Ax, final int Ay, final int Bx, final int By, final int intensity) {
    ...
    void dot(final int x, final int y, final int radius, final boolean filled, final int intensity) {
        if (filled) {
    ...
    private void arc(final int x, final int y, final int innerRadius, final int outerRadius, final int intensity) {
        for (int r = innerRadius; r <= outerRadius; r++) {
            CircleTool.circle(this, x, y, r, intensity);
        }
    }

    private void arc(final int x, final int y, final int innerRadius, final int outerRadius, final int fromArc, final int toArc) {
        for (int r = innerRadius; r <= outerRadius; r++) {
     ...
     */
    private void insertBitmap(final BufferedImage bitmap, final int x, final int y, final int transRGB) {
     ...
     * @param filter filter to be applied
     */
    private void insertBitmap(final BufferedImage bitmap, final int x, final int y, final int transRGB, final FilterMode filter) {
    ...
    private static void demoPaint(final RasterPlotter m) {
        m.setColor(GREY);
    ...
    static ByteArrayOutputStream exportImage(final BufferedImage image, final String targetExt) {
