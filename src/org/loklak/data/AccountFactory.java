            ...
            XContentBuilder mapping = XContentFactory.jsonBuilder()
                .startObject()
                  .startObject("properties") // the id has been omitted on purpose, we identify the user by its screen_name (even if that is changeable..)
                    .startObject(Field.screen_name.name()).field("type","string").field("include_in_all","false").endObject()
                    .startObject(Field.source_type.name()).field("type","string").field("include_in_all","false").endObject()
                    .startObject(Field.authentication_first.name()).field("type","date").field("format","dateOptionalTime").field("include_in_all","false").endObject()
                    .startObject(Field.authentication_latest.name()).field("type","date").field("format","dateOptionalTime").field("include_in_all","false").endObject()
                    .startObject(Field.oauth_token.name()).field("type","string").field("include_in_all","false").endObject()
                    .startObject(Field.oauth_token_secret.name()).field("type","string").field("include_in_all","false").endObject()
